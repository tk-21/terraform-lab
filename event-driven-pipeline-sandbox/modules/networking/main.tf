...
# S3 Gateway（Lambda デプロイパッケージ取得、NAT GW 不使用）
...
# DynamoDB Gateway（複数 Lambda が DynamoDB へアクセス）
...
# Step Functions Interface（Dispatcher Lambda が ExecutionStart を呼ぶ）
...
# SNS Interface（DLQ Handler / Cleanup Lambda が SNS Publish を呼ぶ）
...
# CloudWatch Logs Interface（Lambda → Logs、NAT GW 不使用）
