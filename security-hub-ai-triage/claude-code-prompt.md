...
- `chatwork_secret_arn`（sensitive: true, Chatwork トークンの Secrets Manager ARN）
- `chatwork_room_id`（Chatwork ルーム ID）
- `bedrock_model_id`（default: `"anthropic.claude-haiku-4-5"`）
...
- `secretsmanager:GetSecretValue`（対象リソース: chatwork_secret_arn のみ）
...
- 環境変数: `DYNAMODB_TABLE_NAME`, `S3_BUCKET_NAME`, `CHATWORK_SECRET_ARN`,
  `CHATWORK_ROOM_ID`, `BEDROCK_MODEL_ID` を渡す
...
2. 各 Finding に対して：
   ...
   d. Severity が CRITICAL または HIGH の場合、ChatworkNotifier で通知
...
以下の仕様で実装する：
- `ChatworkNotifier` クラスとして実装
- Secrets Manager から Chatwork トークンを取得（`boto3.client("secretsmanager")`）
- `https://api.chatwork.com/v2/rooms/{room_id}/messages` に POST
- CLAUDE.md のフォーマットで通知メッセージを組み立てる
...
environment          = "dev"
chatwork_secret_arn  = "arn:aws:secretsmanager:ap-northeast-1:123456789012:secret:chatwork-token-xxxx"
...
3. セットアップ手順（Secrets Manager へのトークン登録 → terraform init → apply）
...
- [ ] Chatwork トークンが Secrets Manager 経由で取得されているか（環境変数直書きなし）
