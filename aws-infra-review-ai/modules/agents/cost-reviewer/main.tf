...
#
# 役割: AWS インフラのコスト最適化観点レビューを担当
# チェック観点:
#   - リソース過剰スペック（Lambda メモリ・RDS インスタンスタイプ等）
#   - NAT Gateway 数（コストの高いリソース）
...
locals {
  function_name     = "${var.project_name}-cost-reviewer-${var.environment}"
...
resource "aws_lambda_function" "cost_reviewer" {
